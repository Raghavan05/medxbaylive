...
const Blog = () => {
  ...
  .map(([_, count], i) => {
    const categoryName = count._id?.[0] || "Unknown Category"; // Ensure _id exists and provide a fallback
    const sanitizedCategoryName = categoryName.replace(/\s+/g, '-').toLowerCase(); // Sanitize for valid ID

    return (
      <div key={i}>
        <a href={`#${sanitizedCategoryName}`} onClick={() => scrollToSection(sanitizedCategoryName)}>
          {categoryName}
        </a>
      </div>
    );
